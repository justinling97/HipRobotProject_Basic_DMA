...
//Loadcell Coef for Hip Robot 3
static float LoadCell1_Coef=-212.16f;
static float LoadCell2_Coef=210.91;

static float LoadCell1_Offset=269.0f;
static float LoadCell2_Offset=-266.41f;


#endif

#ifdef HipRobot2
//Loadcell Coef for Hip Robot 2
static float LoadCell1_Coef=-362.71f;
static float LoadCell2_Coef=408.47f;

static float LoadCell1_Offset=456.07f;
static float LoadCell2_Offset=-508.7f;

#endif


#ifdef HipRobot5
//Loadcell Coef for Hip Robot 5
static float LoadCell1_Coef=315.6600f;
static float LoadCell2_Coef=313.1663f;

static float LoadCell1_Offset=-395.7474f;
static float LoadCell2_Offset=-393.3250f;
...
osThreadId MotorRXDecodeG1Handle;
osThreadId MotorRXDecodeG2Handle;
...
void MotorRXDecodeG1FUN(void const * argument);
void MotorRXDecodeG2FUN(void const * argument);
...
void MX_FREERTOS_Init(void) {
  ...
  /* start timers, add new ones, ... */
	...
	osTimerStart(GetRTCTimeHandle,5);
  ...
  /* definition and creation of MotorRXDecodeG1 */
  osThreadDef(MotorRXDecodeG1, MotorRXDecodeG1FUN, osPriorityIdle, 0, 128);
  MotorRXDecodeG1Handle = osThreadCreate(osThread(MotorRXDecodeG1), NULL);

  /* definition and creation of MotorRXDecodeG2 */
  osThreadDef(MotorRXDecodeG2, MotorRXDecodeG2FUN, osPriorityIdle, 0, 128);
  MotorRXDecodeG2Handle = osThreadCreate(osThread(MotorRXDecodeG2), NULL);
...
void TSA1ContollFUN(void const * argument)
{
  ...
  for(;;)
  {
		...
		PositionFlag=0;
			...
			SetMotorCurrentAmpere_Group1(0, 0.3f);       	// ���õ�������Ϊ0.5fA
			...
			SetMotorCurrentAmpere_Group1(1, 0.3f);       	// ���õ�������Ϊ0.5fA
			...
			SetMotorCurrentAmpere_Group1(2, 0.3f);       	// ���õ�������Ϊ0.5fA
			...
			SetMotorCurrentAmpere_Group1(3, 0.3f);       	// ���õ�������Ϊ0.5fA
			SendMotorControlFrame_Group1(); 
			PositionFlag=0;
		}
...
void TSA2ContollFUN(void const * argument)
{
  ...
  for(;;)
  {
		...
		PositionFlag=0;
			...
			SetMotorCurrentAmpere_Group2(0, 0.3f);       	// ���õ�������Ϊ0.5fA
			...
			SetMotorCurrentAmpere_Group2(1, 0.3f);       	// ���õ�������Ϊ0.5fA
			...
			SetMotorCurrentAmpere_Group2(2, 0.3f);       	// ���õ�������Ϊ0.5fA
			...
			SetMotorCurrentAmpere_Group2(3, 0.3f);       	// ���õ�������Ϊ0.5fA
			SendMotorControlFrame_Group2(); 
		...
		if(CurRX_IMU!=0){
			...
			if((CRC_H==IMURX_Buffer[27])&&(CRC_L==IMURX_Buffer[28])){
				if(IMURX_Buffer[0]==0x50){
					...
					IMU_R.AX_Float=((float)IMU_R.AX_int16)*0.0048828125f;
					IMU_R.AY_int16=(IMURX_Buffer[5]<<8)+IMURX_Buffer[6];
					IMU_R.AY_Float=((float)IMU_R.AY_int16)*0.0048828125f;
					IMU_R.AZ_int16=(IMURX_Buffer[7]<<8)+IMURX_Buffer[8];
					IMU_R.AZ_Float=((float)IMU_R.AZ_int16)*0.00478515625f;
					
					
					IMU_R.GX_int16=(IMURX_Buffer[9]<<8)+IMURX_Buffer[10];
					IMU_R.GX_Float=((float)IMU_R.GX_int16)*0.06103515625f;
					IMU_R.GY_int16=(IMURX_Buffer[11]<<8)+IMURX_Buffer[12];
					IMU_R.GY_Float=((float)IMU_R.GY_int16)*0.06103515625f;
					IMU_R.GZ_int16=(IMURX_Buffer[13]<<8)+IMURX_Buffer[14];
					IMU_R.GZ_Float=((float)IMU_R.GZ_int16)*0.06103515625f;
					...
					IMU_R.Roll_Float=((float)IMU_R.Roll_int16)*0.0054931640625f;
					IMU_R.Pitch_int16=(IMURX_Buffer[23]<<8)+IMURX_Buffer[24];
					IMU_R.Pitch_Float=((float)IMU_R.Pitch_int16)*0.0054931640625f;
					IMU_R.Yaw_int16=(IMURX_Buffer[25]<<8)+IMURX_Buffer[26];
					IMU_R.Yaw_Float=((float)IMU_R.Yaw_int16)*0.0054931640625f;
					IMU_R_RSP_Num++;
					...
					IMU_L.AX_Float=((float)IMU_L.AX_int16)*0.0048828125f;
					IMU_L.AY_int16=(IMURX_Buffer[5]<<8)+IMURX_Buffer[6];
					IMU_L.AY_Float=((float)IMU_L.AY_int16)*0.0048828125f;
					IMU_L.AZ_int16=(IMURX_Buffer[7]<<8)+IMURX_Buffer[8];
					IMU_L.AZ_Float=((float)IMU_L.AZ_int16)*0.00478515625f;
					
					
					IMU_L.GX_int16=(IMURX_Buffer[9]<<8)+IMURX_Buffer[10];
					IMU_L.GX_Float=((float)IMU_L.GX_int16)*0.06103515625f;
					IMU_L.GY_int16=(IMURX_Buffer[11]<<8)+IMURX_Buffer[12];
					IMU_L.GY_Float=((float)IMU_L.GY_int16)*0.06103515625f;
					IMU_L.GZ_int16=(IMURX_Buffer[13]<<8)+IMURX_Buffer[14];
					IMU_L.GZ_Float=((float)IMU_L.GZ_int16)*0.06103515625f;
					...
					IMU_L.Roll_Float=((float)IMU_L.Roll_int16)*0.0054931640625f;
					IMU_L.Pitch_int16=(IMURX_Buffer[23]<<8)+IMURX_Buffer[24];
					IMU_L.Pitch_Float=((float)IMU_L.Pitch_int16)*0.0054931640625f;
					IMU_L.Yaw_int16=(IMURX_Buffer[25]<<8)+IMURX_Buffer[26];
					IMU_L.Yaw_Float=((float)IMU_L.Yaw_int16)*0.0054931640625f;
					IMU_L_RSP_Num++;
					memset(IMURX_Buffer,0,30);
					CurRX_IMU=0;
					}
					...
					IMU_Controller.AX_Float=((float)IMU_Controller.AX_int16)*0.0048828125f;
					IMU_Controller.AY_int16=(IMURX_Buffer[5]<<8)+IMURX_Buffer[6];
					IMU_Controller.AY_Float=((float)IMU_Controller.AY_int16)*0.0048828125f;
					IMU_Controller.AZ_int16=(IMURX_Buffer[7]<<8)+IMURX_Buffer[8];
					IMU_Controller.AZ_Float=((float)IMU_Controller.AZ_int16)*0.00478515625f;
					
					
					IMU_Controller.GX_int16=(IMURX_Buffer[9]<<8)+IMURX_Buffer[10];
					IMU_Controller.GX_Float=((float)IMU_Controller.GX_int16)*0.06103515625f;
					IMU_Controller.GY_int16=(IMURX_Buffer[11]<<8)+IMURX_Buffer[12];
					IMU_Controller.GY_Float=((float)IMU_Controller.GY_int16)*0.06103515625f;
					IMU_Controller.GZ_int16=(IMURX_Buffer[13]<<8)+IMURX_Buffer[14];
					IMU_Controller.GZ_Float=((float)IMU_Controller.GZ_int16)*0.06103515625f;
					...
					IMU_Controller.Roll_Float=((float)IMU_Controller.Roll_int16)*0.0054931640625f;
					IMU_Controller.Pitch_int16=(IMURX_Buffer[23]<<8)+IMURX_Buffer[24];
					IMU_Controller.Pitch_Float=((float)IMU_Controller.Pitch_int16)*0.0054931640625f;
					IMU_Controller.Yaw_int16=(IMURX_Buffer[25]<<8)+IMURX_Buffer[26];
					IMU_Controller.Yaw_Float=((float)IMU_Controller.Yaw_int16)*0.0054931640625f;
					IMU_CTL_RSP_Num++;
					memset(IMURX_Buffer,0,30);
					CurRX_IMU=0;
					}
...
/* USER CODE BEGIN Header_MotorRXDecodeG1FUN */
/**
* @brief Function implementing the MotorRXDecodeG1 thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_MotorRXDecodeG1FUN */
void MotorRXDecodeG1FUN(void const * argument)
{
  /* USER CODE BEGIN MotorRXDecodeG1FUN */
  /* Infinite loop */
for(;;)
  {
				osStatus status = osSemaphoreWait(Motor1RetStateHandle, osWaitForever);
        if (status == osOK)
        {
					ProcessFeedbackFrame_Group1(&Motor1_CTRLBuffer[0]);
        }
  }
  /* USER CODE END MotorRXDecodeG1FUN */
}

/* USER CODE BEGIN Header_MotorRXDecodeG2FUN */
/**
* @brief Function implementing the MotorRXDecodeG2 thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_MotorRXDecodeG2FUN */
void MotorRXDecodeG2FUN(void const * argument)
{
  /* USER CODE BEGIN MotorRXDecodeG2FUN */
  /* Infinite loop */
  for(;;)
  {
				osStatus status = osSemaphoreWait(Motor2RetStateHandle, osWaitForever);
        if (status == osOK)
        {
					ProcessFeedbackFrame_Group2(&Motor2_CTRLBuffer[0]);
        }
  }
  /* USER CODE END MotorRXDecodeG2FUN */
}
...
void LoadCellCALFUN(void const * argument)
{
  /* USER CODE BEGIN LoadCellCALFUN */
	...
	/*   FIR FILTER */
	LoadCell1Buffer[0]=(float32_t)ADC_DATA[0]/16383.0f*3.3f;
	LoadCell2Buffer[0]=(float32_t)ADC_DATA[2]/16383.0f*3.3f;
	
	PC0_VolVal=(float32_t)ADC_DATA[4]/16383.0f*3.3f;
	PA5_VolVal=(float32_t)ADC_DATA[5]/16383.0f*3.3f;
	...
	#ifdef HipRobot3
	LoadCellForce[1]=LoadCell2_Coef*LoadCell2Buffer_Filtered[0]+LoadCell2_Offset;
	LoadCellForce[0]=LoadCell1_Coef*LoadCell1Buffer_Filtered[0]+LoadCell1_Offset;
	#endif
	#ifdef HipRobot2
	LoadCellForce[0]=LoadCell2_Coef*LoadCell2Buffer_Filtered[0]+LoadCell2_Offset;
	LoadCellForce[1]=LoadCell1_Coef*LoadCell1Buffer_Filtered[0]+LoadCell1_Offset;
	#endif
