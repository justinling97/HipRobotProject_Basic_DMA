...
// 全局变量定义
extern uint8_t tempMotor1_CTRLBuffer[Motor_Buffer_Len];
extern uint8_t tempMotor2_CTRLBuffer[Motor_Buffer_Len];
...
__attribute__((section(".RAM_D2"))) __attribute__((aligned(32))) uint8_t g_txBuffer_Group1[FRAME_TOTAL_SIZE];  // 发送缓冲区
__attribute__((section(".RAM_D2"))) __attribute__((aligned(32))) uint8_t g_txBuffer_Group2[FRAME_TOTAL_SIZE];  // 发送缓冲区
...
void SendMotorControlFrame_Group1(void)
{
    ...
    // 通过UART5发送数据帧
		SCB_CleanDCache_by_Addr((uint32_t*)g_txBuffer_Group1, FRAME_TOTAL_SIZE);
		HAL_UART_Transmit_DMA(&huart5, g_txBuffer_Group1, ProtocolLen);
}
...
void SendMotorControlFrame_Group2(void)
{
    ...
    // 通过UART5发送数据帧
		SCB_CleanDCache_by_Addr((uint32_t*)g_txBuffer_Group2, FRAME_TOTAL_SIZE);
		HAL_UART_Transmit_DMA(&huart7, g_txBuffer_Group2, ProtocolLen);
}
...
void ProcessFeedbackFrame_Group1(uint8_t *data)
{
    ...
    // 验证校验和
    uint8_t checksum = CalculateChecksum(data, ProtocolLen - 1);
    if(checksum != data[ProtocolLen - 1]) {
        return;
    }
...
void Motor1_StartRxToIdle_DMA(void)
{
    /* 为避免Cache里脏数据覆盖DMA写入，启动前清理/失效该区域 */
    SCB_InvalidateDCache_by_Addr((uint32_t*)&tempMotor1_CTRLBuffer[0], DCACHE_ALIGN(Motor_Buffer_Len));

    HAL_UARTEx_ReceiveToIdle_DMA(&huart5,&tempMotor1_CTRLBuffer[0],Motor_Buffer_Len); 
    /* 关闭半传输中断(可选)，避免HT回调干扰 */
    __HAL_DMA_DISABLE_IT(huart5.hdmarx, DMA_IT_HT);
}

void Motor2_StartRxToIdle_DMA(void)
{
    /* 为避免Cache里脏数据覆盖DMA写入，启动前清理/失效该区域 */
    SCB_InvalidateDCache_by_Addr((uint32_t*)&tempMotor2_CTRLBuffer[0], DCACHE_ALIGN(Motor_Buffer_Len));

    HAL_UARTEx_ReceiveToIdle_DMA(&huart7,&tempMotor2_CTRLBuffer[0],Motor_Buffer_Len); 
    /* 关闭半传输中断(可选)，避免HT回调干扰 */
