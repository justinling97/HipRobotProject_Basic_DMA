...
__attribute__((section(".RAM_D2"))) __ALIGNED(32) uint8_t tempMotor1_CTRLBuffer[Motor_Buffer_Len];
extern uint8_t Motor1_Return_Data[32];

extern uint8_t Motor2_CTRLBuffer[Motor_Buffer_Len];
__attribute__((section(".RAM_D2"))) __ALIGNED(32) uint8_t tempMotor2_CTRLBuffer[Motor_Buffer_Len];
...
int main(void)
{
  ...
  Motor1_StartRxToIdle_DMA();//Receive the return state of motor 1
  Motor2_StartRxToIdle_DMA();//Receive the return state of motor 2
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
	...
	if(huart->Instance==UART5)
	{
		...
		SCB_InvalidateDCache_by_Addr((uint32_t*)tempMotor1_CTRLBuffer, DCACHE_ALIGN(Motor_Buffer_Len));
		memcpy(Motor1_CTRLBuffer, tempMotor1_CTRLBuffer, Motor_Buffer_Len);
		osSemaphoreRelease(Motor1RetStateHandle);
		Motor1_StartRxToIdle_DMA();
		...
		if(huart->Instance==UART7)
	{
		...
		osSemaphoreRelease(Motor2RetStateHandle);
		Motor2_StartRxToIdle_DMA();
	}
