...
int32_t WitReadReg(uint8_t IMUID,uint32_t uiReg, uint32_t uiReadNum)
{
    uint16_t usTemp, i;
    uint8_t ucBuff[8];
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_MODBUS:
            ...
            IMU_TX_Buf[0] = IMUID;
            IMU_TX_Buf[1] = FuncR;
            IMU_TX_Buf[2] = uiReg >> 8;
            IMU_TX_Buf[3] = uiReg & 0xFF;
            IMU_TX_Buf[4] = uiReadNum >> 8;
            IMU_TX_Buf[5] = uiReadNum & 0xff;
            usTemp = __CRC16(IMU_TX_Buf, 6);
            IMU_TX_Buf[6] = usTemp >> 8;
            IMU_TX_Buf[7] = usTemp & 0xff;
						//HAL_UART_Transmit_DMA(&huart3, &IMU_TX_Buf[0], 8);
            //p_WitSerialWriteFunc(IMU_TX_Buf, 8);
            break;
        case WIT_PROTOCOL_CAN:
